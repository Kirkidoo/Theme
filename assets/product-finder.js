document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!section) {
        // Logic for collection page initialization
        if (!section) {
            setTimeout(updateGlobalBadges, 500);
        ...
        els.reset.addEventListener('click', function () {
            ...
            localStorage.removeItem(RESULTS_CACHE_KEY);
            updateGlobalBadges(); // Clear badges
        });
        ...
        els.submit.addEventListener('click', async function () {
            ...
            toggleGarageView(true);
            updateGlobalBadges(); // Show badges for new vehicle
        ...
        function renderCachedCards(cachedItems) {
            ...
            cachedItems.forEach((item, idx) => {
                ...
                if (item.found) {
                    ...
                    const fitmentBtn = `<button class="pf-fitment-btn" type="button" data-sku="${item.sku}" data-title="${item.title.replace(/"/g, '&quot;')}">Fitment</button>`;
                    const specsBtn = `<button class="pf-specs-btn" type="button" data-sku="${item.sku}" data-title="${item.title.replace(/"/g, '&quot;')}" data-variant-id="" data-available="false" data-inventory-qty="0" data-inventory-policy="" data-inventory-management="">Specs</button>`;
                    const actionRow = `<div class="pf-action-row">${specsBtn}${fitmentBtn}</div>`;
        ...
        function updateCard(idx, gammaItem, data, arrivalData, gammaDetails) {
            ...
            // --- SCENARIO 2: NOT FOUND (SPECIAL ORDER) ---
                ...
                const specsBtn = `<button class="pf-specs-btn" type="button" data-sku="${cacheObj.sku}" data-title="${cacheObj.title.replace(/"/g, '&quot;')}" data-variant-id="" data-available="false" data-inventory-qty="0" data-inventory-policy="" data-inventory-management="">Specs</button>`;
                const actionRow = `<div class="pf-action-row">${specsBtn}${fitmentBtn}</div>`;
        ...
        async function updateGlobalBadges() {
            ...
            const badges = document.querySelectorAll('.pf-collection-badge');
